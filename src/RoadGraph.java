...
public class RoadGraph {
      ...
      public boolean osmGraphParser(XmlPullParser xrp) throws XmlPullParserException, IOException {
            ...
            LinkedList allWays = new LinkedList();
                              ...
                              if (xrp.getName().equals("osm")) {
                                    ...
                                    if (xrp.getName().equals("node")) {
                                          ...
                                          if (tempNode == NULL_NODE) {
                                                for (int i = 0; i < attributeCount; i++) {
                                                      if (xrp.getAttributeName(i).equals("k") && xrp.getAttributeValue(i).equals("highway")) {
                                                            String v = xrp.getAttributeValue(i + 1);
                                                            tempWay.setType(v);
                                                            tempWay.setSpeedMax(OsmConstants.roadTypeToSpeed(v));
                                                      } else if (xrp.getAttributeName(i).equals("k") && xrp.getAttributeValue(i).equals("name")) {
                                                            String v = xrp.getAttributeValue(i + 1);
                                                            tempWay.setName(v);
                                                      } else if (xrp.getAttributeName(i).equals("k") && xrp.getAttributeValue(i).equals("other_tags")) {
                                                            String v = xrp.getAttributeValue(i + 1);
            ...
            LinkedList remainingWays = new LinkedList();
            for (Object allWay : allWays) {
            }
            for (Object gway : allWays) {
                  GraphWay way = GraphWay.class.cast(gway);
                  LinkedList refs = way.getRefs();
                  boolean stop = false;
                  for (Object lref : refs) {
                        Long ref = Long.class.cast(lref);
                        for (Object gnode : allNodes){
                              GraphNode node = GraphNode.class.cast(gnode);
                              if (node.getId() == ref) {
      ...
      private GraphNode getNode(LinkedList list, long id) {
            for (Object gnode : list) {
                  GraphNode node = GraphNode.class.cast(gnode);
                  if (node.getId() == id) {
                        return node;
                  }
